...
const DATA_DIR = z.string().min(1).default("data").parse(Deno.env.get("DATA_DIR"));
const DB_FILE_NAME = z.string().min(1).default("denokv.sqlite3").parse(Deno.env.get("DB_FILE_NAME"));
...
export function getCounterPath(namespace: number, counter: number): string {
  return resolve(DATA_PATH, namespace.toString(), `${counter.toString().padStart(8, "_")}.log`);
}
...
export async function ensureFileContent(path: string, content: string): Promise<void> {
  try {
